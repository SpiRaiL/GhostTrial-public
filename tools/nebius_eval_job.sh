...
echo "=== [4/5] evaluate, with rendering ==="
# Upstream's own eval entry point, not a bespoke replay loop. Three measurement
# bugs came out of maintaining our own; this is the path NVIDIA tests.
#
# Rendering is enabled here and NOT locally on purpose: Isaac Sim 5.1 inside Isaac
# Lab 2.3.2 cannot start the RTX renderer on a Blackwell card (our RTX 5080), but a
# Nebius H100 is Hopper, so the renderer comes up.
#
# Hydra rules that have bitten before: eval reads its config from beside the
# checkpoint, so NO +exp=; keys base_eval does not declare need a leading +.
mkdir -p /opt/gr00t/logs_rl
$PY_BIN gear_sonic/eval_agent_trl.py \
  +checkpoint="${CKPT_PATH}" \
  +num_envs="${NUM_ENVS:-4}" +headless=True \
  ++render_results=True \
  ++algo.config.eval.save_videos=True \
  ++algo.config.eval.num_eval_episodes="${EPISODES:-4}" \
  ++algo.config.eval.save_goal_reached_only=False \
  ++algo.config.eval.video_save_prob=1.0 \
  ++max_render_steps="${RENDER_STEPS:-600}" \
  ++manager_env.commands.motion.motion_lib_cfg.motion_file=/opt/gr00t/motion \
  ++manager_env.commands.motion.motion_lib_cfg.smpl_motion_file=dummy || EVAL_RC=$?

echo "=== [5/5] sync renderings ==="
$PY_BIN - <<'PYEOF'
...
n = 0
for f in ["/tmp/job_output.log"]:
    if os.path.isfile(f):
        s3.upload_file(f, os.environ["NB_BUCKET"], f"eval/{run}/job_output.log")
        n += 1
for pat in ("/opt/gr00t/**/renderings/**/*", "/opt/gr00t/logs_rl/**/*",
            "/opt/gr00t/**/*.mp4", "/opt/gr00t/**/*.png"):
    for f in glob.glob(pat, recursive=True):
        if os.path.isfile(f) and os.path.getsize(f) < 2_000_000_000:
            s3.upload_file(f, os.environ["NB_BUCKET"],
                           f"eval/{run}/" + os.path.relpath(f, "/opt/gr00t"))
            n += 1
